...
- **выборочное восстановление**: всё (БД + файлы), только база данных или только файлы — отдельно для панели и для бота
- имена файлов бэкапа отражают содержимое (`full` / `panel` / `bot`) — сразу видно что внутри
...
- тест подключения к S3 и тест прокси для Telegram API с подробной диагностикой ошибок
- настраиваемая политика хранения бэкапов на сервере (в днях или часах) и S3 в отдельности
- при удалении скрипта можно сохранить локальные бэкапы в отдельную директорию
...
## О выборочном восстановлении

Если в бэкапе присутствуют и база данных, и файлы (для панели или для бота), скрипт предложит выбрать, что восстановить:

- **Всё** — БД и файлы вместе
- **Только база данных** — том БД пересоздаётся с нуля, файлы панели/бота не трогаются. Требует, чтобы директория с уже установленной панелью/ботом существовала
- **Только файлы** — заменяются файлы и конфигурация, база данных не трогается

Это удобно, например, если нужно откатить только базу после неудачного обновления, не трогая текущие настройки и файлы.
